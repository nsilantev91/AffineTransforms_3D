...
namespace AffineTransforms_3D
{
    class Lighting
    {
        ...
        public static Bitmap lighting(int width, int height, Figure figure, Point3D lightPoint)
        {
            Bitmap bmp = new Bitmap(width, height);
            FastBitmap fastBmp = new FastBitmap(bmp);
            fastBmp.Lock();
            fastBmp.Clear(Color.White);
            fastBmp.SetPixel((int)lightPoint.X, (int)lightPoint.Y, Color.Red);
            fastBmp.SetPixel((int)lightPoint.X + 1, (int)lightPoint.Y, Color.Red);
            fastBmp.SetPixel((int)lightPoint.X, (int)lightPoint.Y + 1, Color.Red);
            fastBmp.SetPixel((int)lightPoint.X + 1, (int)lightPoint.Y + 1, Color.Red);
            fastBmp.Unlock();
            ...
            {
                rastFigure.Add(triag.Key, Rasterize(triag.Value[0], lightPoint));
                for (int i = 1; i < triag.Value.Count; i++)
                {
                    rastFigure[triag.Key].AddRange(Rasterize(triag.Value[i], lightPoint));
                }
            }        
            var centerX = width / 2;
        ...
        static double lambertModel(Point3D vertex, Vector3D normal, Point3D lightPoint)
        {
        ...
        static public List<Tuple<double,Point3D>> Rasterize(List<Edge> triangle, Point3D lightPoint)
        {
        ...
        static public List<Tuple<double, double, double, Point3D>> RasterizeWithUV(List<Edge> triangle, Point3D lightPoint, int xMin, int yMin, int xMax, int yMax)
        {
            List<Point3D> points = new List<Point3D>();
            foreach (var edge in triangle)
            {
        ...
        public static Bitmap texturing(int width, int height, Figure figure, Point3D lightPoint, bool withLight = false)
        {
            Bitmap bmp = new Bitmap(width, height);
            FastBitmap fastBmp = new FastBitmap(bmp);
            fastBmp.Lock();
            ...
            foreach (var triag in triags)
            {
                ...
                rastFigure.Add(triag.Key, RasterizeWithUV(triag.Value[0], lightPoint, xMin, yMin, xMax, yMax));
                for (int i = 1; i < triag.Value.Count; i++)
                {
                    rastFigure[triag.Key].AddRange(RasterizeWithUV(triag.Value[i], lightPoint, xMin, yMin, xMax, yMax));
                }
            }
            var centerX = width / 2;
            var centerY = height / 2;
